...
To build a package, first the mirbase (http://www.mirbase.org) files for the desired version have to be downloaded.
Subsequently, the genomic alignments provided from the mirbase are used to determine whether at that location a gene is defined in the Ensembl database. This step defines the host genes and transcripts.
In the next steps, a table with the pre-miRNA sequences is generated, as well as a table providing the mapping of pre-miRNAs to miRNA families (based on the information from the mirbase).
Based on the tables generated from the functions above, an SQLite database is generated, for which in the last step the database package is created.
...
+ [ ] Analyze the /GSE25320/ data set.
+ [ ] Analyze the /GSE47379/ data set.
+ [ ] Analyze the /GSE53384/ data set.

** TODO Fix and update documentation

+ [ ] Remove unnecessary /value/ section.
...
** TODO Database creation methods: replace all =cat= commands in the methods by =message=.
